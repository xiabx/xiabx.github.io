...
如果直接抛出去会有两种麻烦，第一由于DAO层使用的技术选择性很多，可能面临更换技术的情况，比如由JDBC改为了使用内存型数据库的情况，那么原先抛出去的JDBC类型的异常的处理已经耦合到Service层中，不好处理。第二个问题就是由于之前Service层只是处理了JDBC类型的异常，若引入了别的异常则需要再改动Service层代码，让其可以处理新的异常类型。这样也破坏了mvc的分层结构。

若直接在DAO层处理了异常信息，那么对于Service来说就不知道数据访问期间发生了什么问题。

处理这个问题的方式就是将特定的数据访问异常进行封装，封装后的异常类型应选择unchecked exception。这是因为数据库的异常通常无法在Service层进行处理，如链接失败、无法取得相应资源等。而采用unchecked exception后DAO层接口也不会声明式抛出异常信息。
...
在org.springframework.dao包下spring预定义了一些数据库异常信息类。他们均为DataAccessException子类。在JDBCTemplate中将通过SQLExceptionTranslator根据异常信息进行解析后抛出相应的异常。
...
JdbcTemplate中各种模板方法按照其通过相应Callback接口所公开的API自由程度的大小，这几个方法都属于jdbc.execute方法的重载，可以分为如下4组：
...
![image-20200304115658086](https://xbxblog2.bj.bcebos.com/springJdbc%2Fimage-20200304115658086.png)
...
SQLExceptionTranslator在jdbcTemplate中使用如下，当catch住异常后会通过SQLExceptionTranslator将异常信息转译，然后抛出预定义的异常。

![image-20200304131637097](https://xbxblog2.bj.bcebos.com/springJdbc%2Fimage-20200304131637097.png)
