...
该方法通过委托PostProcessorRegistrationDelegate类的invokeBeanFactoryPostProcessors方法进行执行。主要思路为先遍历硬编码的BeanFactoryPostProcessors，即通过AbstractApplicationContext的addBeanFactoryPostProcessors方法添加的，但是。。。没找到在哪里添加的。

然后就是自定义的BeanFactoryPostProcessors执行了，主要原理是通过对beanFactory中注册bean，只要是实现了对应接口就执行。。。
